...
Frase de abertura padrão: quem é o cliente (uma linha), quem executa pela Moinho, e o lembrete de ler o arquivo antes de qualquer tarefa.
...
Foco **só na marca/empresa em si**: o que ela é, produto ou serviço em uma linha, diferenciais, como quer ser percebida no mercado, site, contatos, redes, tagline, tom de voz da marca, segmento, concorrentes.

- **Não misturar informação do produto vendido aqui** — isso é assunto de "Produtos/Serviços". Um contexto já trouxe detalhe de assinatura (preço, o que inclui) dentro de "Quem é" e teve que ser realocado.
- Basear em material oficial da marca quando existir (ex.: branding entregue por agência parceira), nunca supor.
- **Concorrentes:** quando o cliente atua em mais de uma frente, separar o concorrente por produto/serviço. Uma empresa pode revender um produto do cliente numa frente e competir diretamente em outra — listar como "concorrente" genérico esconde essa nuance.
...
- Esse tópico já existiu com o nome "Esteira de produtos" — o nome padronizado é este.
...
- Puxar personas já validadas em planejamento ou onboarding em vez de recriar do zero.
...
- Esse fluxo já apareceu escondido dentro de "Canais e tráfego" em mais de um contexto — é tópico próprio, não um parágrafo solto em outra seção.
...
- Tem que refletir o que está **sendo executado de fato**, não uma fase antiga ou um plano que mudou. Um contexto ficou descrevendo a estratégia de uma campanha anterior (e-book) quando na prática o foco já tinha virado outro (landing page) — isso gera relatório e briefing errados.
...
- Esse tópico muda com frequência (campanhas pausam, nascem, são renomeadas). Sinalizar no próprio arquivo que ele precisa de revisão periódica — não é "escreve uma vez e esquece".
...
Mapa de onde estão os materiais de origem (branding, planejamento, onboarding, calendário, identidade visual), com caminho local e/ou link do Drive.
